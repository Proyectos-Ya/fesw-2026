...
from tests.matching_evaluation.evaluate_named_vectors import MultiVectorTextBuilder, cosine_similarity, load_all_tenders
...
async def run_sensitivity_analysis():
    ...
    print("🔬 ANÁLISIS DE SENSIBILIDAD Y BÚSQUEDA DE PONDERACIONES ÓPTIMAS (GRID SEARCH)", flush=True)
    print("=" * 100, flush=True)
    ...
    for i, t in enumerate(valid_candidates):
        ...
        raw_sims[t.id] = {
            "s_ov": s_ov, "s_it": s_it, "s_req": s_req,
    ...
    reranked = await reranker.rerank(query_text=supplier_global_text, candidates=rerank_pairs, limit=len(valid_candidates))
    ...
    CORE_CODES = ["1101892-152-COT26", "3113-38-COT26", "4066-312-COT26", "1593-148-COT26", "5504-260-COT26"]
        ...
        for (W_vec, W_rr, W_sec, W_kw) in hybrid_weight_configs:
            for mode in scaling_modes:
                ...
                for t in valid_candidates:
                    sim = raw_sims[t.id]
                    if mode == "scaled":
                        v_score = (w_ov * sim["sc_ov"]) + (w_it * sim["sc_it"]) + (w_req * sim["sc_req"])
                    else:
                        v_score = (w_ov * sim["s_ov"]) + (w_it * sim["s_it"]) + (w_req * sim["s_req"])
                ...
                core_scores = [scores_by_code.get(c, 0.0) for c in CORE_CODES if c in scores_by_code]
                sec_scores = [scores_by_code.get(c, 0.0) for c in SECONDARY_CODES if c in scores_by_code]
    ...
    print("\n" + "=" * 100, flush=True)
    print("📊 RESULTADOS DEL GRID SEARCH - TOP 5 COMBINACIONES DE PESOS ENCONTRADAS", flush=True)
    print("=" * 100, flush=True)

    for i, exp in enumerate(all_experiments[:5], 1):
        w_ov, w_it, w_req = exp["v_weights"]
        W_vec, W_rr, W_sec, W_kw = exp["h_weights"]
        print(f"Opción #{i} | Puntuación de Calidad: {exp['metric']:.4f} | Modo: {exp['mode'].upper()}", flush=True)
        print(f"   📐 Named Vectors: Overview={w_ov*100:.0f}%, Items={w_it*100:.0f}%, Requisitos={w_req*100:.0f}%", flush=True)
        print(f"   ⚙️ Pesos Híbridos: MultiVector={W_vec*100:.0f}%, Reranker={W_rr*100:.0f}%, Sector={W_sec*100:.0f}%, Keywords={W_kw*100:.0f}%", flush=True)
        print(f"   🎯 Promedio Licitaciones Core: {exp['avg_core']*100:.1f}% (Mínimo: {exp['min_core']*100:.1f}%) | Licitaciones Secundarias: {exp['avg_sec']*100:.1f}%", flush=True)
        print("   Detalle de Licitaciones Clave:")
        for c in CORE_CODES:
            if c in exp["scores"]:
                print(f"      • [{c}]: {round(exp['scores'][c]*100)}% (Score: {exp['scores'][c]:.4f})", flush=True)
        print("-" * 100, flush=True)
    ...
    sorted_tenders = sorted(valid_candidates, key=lambda t: best["scores"].get(t.code, 0.0), reverse=True)
    for rank, t in enumerate(sorted_tenders[:10], 1):
        score = best["scores"].get(t.code, 0.0)
        percentage = round(score * 100)
        sec_m, kw_m = lexical_flags[t.id]
        flags = []
        if sec_m: flags.append("Sector")
        if kw_m: flags.append("Keywords")
        flags_str = ", ".join(flags) if flags else "Semántico"
        amount_str = f"${t.available_amount_clp:,.0f} CLP" if t.available_amount_clp else "N/A"
        print(f"#{rank:02d} | [{t.code}] | Compatibilidad: {percentage}% (Score: {score:.4f})", flush=True)
        print(f"     Nombre: {t.name}", flush=True)
        print(f"     Comprador: {t.buyer_name or 'N/A'} | Coincidencias: [{flags_str}] | Monto: {amount_str}", flush=True)
        print("-" * 100, flush=True)
