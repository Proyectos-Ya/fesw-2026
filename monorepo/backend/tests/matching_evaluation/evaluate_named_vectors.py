...
class MultiVectorTextBuilder:
    ...
    def build_tender_texts(tender: Tender) -> tuple[str, str, str]:
        ...
        if tender.items:
            items_str = ". ".join(
                [
                    ...
                    + (
                        f" ({i.description})"
                        if i.description and i.description != i.name
                        else ""
                    )
        ...
        if tender.description:
            req_parts.append(
                f"Términos de referencia y requerimientos: {tender.description}"
            )
        requirements = (
            ". ".join(req_parts)
            if req_parts
            else f"Requisitos de contratación para {tender.name}"
        )
    ...
    def build_supplier_texts(supplier: Supplier) -> tuple[str, str, str]:
        ...
        items = (
            f"Catálogo de productos, servicios y partidas técnicas: {kw_str}".strip()
        )
        ...
        certs_str = (
            ...
            else "Sin certificaciones especiales requeridas"
        )
...
async def load_all_tenders() -> dict[UUID, Tender]:
    conn = await asyncpg.connect(
        database=PG_DB, user=PG_USER, password=PG_PASSWORD, host=PG_HOST, port=PG_PORT
    )
...
async def evaluate_with_named_vectors(
    ...
    for i, t in enumerate(valid_candidates):
        ...
        multi_vec_score = (
            (w_requirements * s_req) + (w_overview * s_ov) + (w_items * s_it)
        )
    ...
    reranked = await reranker.rerank(
        query_text=supplier_global_text, candidates=rerank_pairs, limit=20
    )
    ...
    for t, m_score in top_candidates:
        ...
        sector_matched = any(
            s.lower() in all_text.lower() for s in (supplier.sectors or [])
        )
        keyword_matched = any(
            kw.lower() in all_text.lower() for kw in (supplier.keywords or [])
        )
    ...
    results = []
    for rank, (t, final_score, rr_score, s_dict, sec_m, kw_m) in enumerate(
        final_scored[:top_k], 1
    ):
...
async def main():
    ...
    print(
        "🎯 EVALUACIÓN DE PONDERACIÓN CON NAMED VECTORS (w1*req + w2*overview + w3*items)",
        flush=True,
    )
    print("=" * 100, flush=True)
    ...
    print(
        "[MODELS] Inicializando SentenceTransformer BGE-M3 y Reranker ONNX...",
        flush=True,
    )
    ...
    print(
        "Ponderaciones Named Vectors: w_overview=0.40, w_items=0.40, w_requirements=0.20",
        flush=True,
    )
    ...
    print("\n" + "=" * 100, flush=True)
    print(
        "🏆 RESULTADOS TOP 10 CON PONDERACIÓN MULTI-VECTOR + RERANKER HÍBRIDO",
        flush=True,
    )
    print("=" * 100, flush=True)
    ...
    for r in res["results"]:
        ...
        print(
            f"#{r['rank']:02d} | [{r['code']}] | Compatibilidad: {r['final_percentage']} (Final Score: {r['final_score']})",
            flush=True,
        )
        print(f"     Nombre: {r['name'][:80]}", flush=True)
        print(
            f"     📊 Multi-Vector: {r['multi_vec_score']} [Overview (40%): {r['s_overview']} | Items (40%): {r['s_items']} | Requisitos (20%): {r['s_requirements']}]",
            flush=True,
        )
        print(
            f"     🔍 Reranker: {r['reranker_score']} | Coincidencias: [{flags_str}] | Monto: {amount_str}",
            flush=True,
        )
        print("-" * 100, flush=True)
