...
async def main():
    ...
    for _, row in df_b.iterrows():
        ...
        b_reg = (
            int(row["region_id"])
            if "region_id" in row and not pd.isna(row["region_id"])
            else None
        )
        b_created = clean_val(row.get("created_at")) or now_naive
        b_updated = clean_val(row.get("updated_at")) or now_naive
        await conn.execute(
            "INSERT INTO buyer_institution (rut, name, region_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (rut) DO NOTHING;",
            b_rut,
            b_name,
            b_reg,
            b_created,
    ...
    for _, row in df_t.iterrows():
        ...
        t_buyer_unit = (
            str(row["buyer_unit"]) if not pd.isna(row["buyer_unit"]) else None
        )
        t_amount = (
            float(row["available_amount_clp"])
            if not pd.isna(row["available_amount_clp"])
            else None
        )
    ...
    for _, row in df_ti.iterrows():
        ...
        ti_prod_code = (
            str(row["product_code"]) if not pd.isna(row["product_code"]) else ""
        )
        ...
        ti_unit = (
            str(row["unit_of_measure"]) if not pd.isna(row["unit_of_measure"]) else "UN"
        )
