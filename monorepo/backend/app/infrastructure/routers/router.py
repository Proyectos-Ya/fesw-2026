...
def create_router(get_supplier_repo: Callable, get_supplier_vector_repo: Callable) -> APIRouter:
    """Ensambla todos los sub-routers con sus dependencias inyectadas."""
    root = APIRouter()

    root.include_router(create_health_router(), tags=["Health"])
    root.include_router(create_supplier_router(get_supplier_repo=get_supplier_repo, 
