...
class TenderScheduler:
    ...
    async def start_metadata_loop(self) -> None:
        ...
        while True:
            now = datetime.now()
            # Calculamos la espera hasta las 02:00 AM del día siguiente
            tomorrow = (now + timedelta(days=1)).replace(
                hour=2, minute=0, second=0, microsecond=0
            )
            wait_seconds = (tomorrow - now).total_seconds()
            print(f"[Scheduler] Próxima descarga de metadatos programada en {wait_seconds/3600:.2f} horas (a las 02:00 AM)")
    ...
    async def start_processing_loop(self) -> None:
        print("[Scheduler] Iniciando loop de procesamiento de detalles crudos (cada 2 segundos)...")
        while True:
            try:
