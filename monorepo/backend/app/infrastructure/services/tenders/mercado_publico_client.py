...
class MercadoPublicoClient:
    ...
    async def get_tenders(self, from_date: datetime, to_date: datetime, quantity: int) -> List[Dict[str, Any]]:
        headers = {
            "ticket": self.api_key
        }
        ...
        async with httpx.AsyncClient() as client:
            while len(all_items) < quantity:
                ...
                try:
                    print(f"[API MP] Consultando página {current_page} (listado de cambios) a {self.base_url}...")
                    ...
                    print(f"[HTTP Error MP] Error en página {current_page} al listar licitaciones: {http_err.response.status_code}")
                    break
                except Exception as e:
                    print(f"[Error MP] Falla inesperada en página {current_page} al listar licitaciones: {e}")
                    break
    ...
    async def get_tender_detail(self, id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                print(f"[HTTP Error MP] Error al obtener detalle de {id}: {http_err.response.status_code}")
                return {}
            except Exception as e:
